...
@passwordResetBlueprint.route("/passwordreset/codesent=<codeSent>", methods=["GET", "POST"])
    ...
    if codeSent == "true":
        ...
        if request.method == "POST":
            ...
            if code == passwordResetCode:
                cursor.execute(f'select password from users where lower(userName) = "{userName.lower()}"')
                ...
                if password == passwordConfirm:
                    if sha256_crypt.verify(password, oldPassword):
                        ...
                        cursor.execute(f'update users set password = "{password}" where lower(userName) = "{userName.lower()}"')
                        connection.commit()
                        messageDebugging("2", f'USER: "{userName}" CHANGED HIS PASSWORD')
        ...
        if request.method == "POST":
            ...
            cursor.execute(f'select * from users where lower(email) = "{email.lower()}"')
            emailDB = cursor.fetchone()
            
            if not userNameDB or not emailDB:
                port = 587
                ...
                server.ehlo()
                server.login("softwarebank@gmail.com", "lsooxsmnsfnhnixy")
                ...
                server.quit()
                messageDebugging("2", f'PASSWORD RESET CODE: "{passwordResetCode}" SENT TO "{email}"')
